...
- **互联网公开文档**：公开链接直接传 `--node`；文档开启了密码保护时，通过 `--password <ACCESS_PASSWORD>` 提供访问密码，普通文档无需传入。密码只进入读取请求，不会回显在读取结果里，`--dry-run` 预览中固定显示为 `[REDACTED]`。注意：密码以命令行参数传入时会留在 shell 历史与进程列表中，共享环境下注意清理。
